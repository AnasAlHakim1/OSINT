...
const getDataNodes = async (dataID, ID, nodeNum) => {
  try {
    ...
    await axios.get(`http://ec2-54-197-104-168.compute-1.amazonaws.com/api/folder/${dataID}/`).then((res) => {
      if (res.status === 200) {
...
const getDataNode = async (dataID, ID, nodeNum) => {
  try {
    ...
    await axios.get(`http://ec2-54-197-104-168.compute-1.amazonaws.com/api/root/${dataID}/`).then((res) => {
      if (res.status === 200) {
        if (res.data.length === 0) {
          error.value = true;
          setTimeout((_) => (error.value = false), 5000);
...
onMounted(_ => {
  try {
    axios.get("http://ec2-54-197-104-168.compute-1.amazonaws.com/api/root/").then((res) => {
      if (res.status === 200) {
        res.data.forEach((e, i) => {
          root.value.push(
            {
